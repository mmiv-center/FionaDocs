                ...
                while True:
                        ...
                        if not response:
                                ...
                                if not os.path.exists(patientdir):
                                        ...
                                        os.umask(oldmask)
                                # create a folder for structured reports based on StudyInstanceUID
                                srdir = datadir + '/site/srs'
                                ...
                                if dataset.PatientID:
                                        ...
                                        if not os.path.exists(patdir):
                                                ...
                                                os.umask(oldmask)
                                        studydir = os.path.join(patdir, "unknown_unknown")
                                        try:
                                ...
                                if "Modality" in dataset and dataset.Modality == "SR":
                                        try:
                                                manu = "_"
                                                if dataset.Manufacturer:
                                                        manu = dataset.Manufacturer
                                                srdir_path = os.path.join(srdir, manu, dataset.StudyInstanceUID)
                                                if not os.path.exists(srdir_path):
                                                        oldmask = os.umask(0)
                                                        os.makedirs(srdir_path,0o1777)
                                                        os.umask(oldmask)
                                                sr_file_path = os.path.join(srdir_path, dataset.SOPInstanceUID)
                                                if not os.path.islink( sr_file_path ):
