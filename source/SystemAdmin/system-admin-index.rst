...
The database server running MariaDB should be able to scale based on the detailed logging information generated by REDCap for its 21 CFR Part 11 compliance. At our institutions the system is running with 200GB main memory (5 years of operation) and an off-system, compressed, full database backup.
...
Log information is stored in REDCap for data viewing, export, change, and delete operations specific to REDCap. Such information is stored in the relational database tables *redcap_log_event* and *redcap_log_view* and can be accessed using SQL as well as by using the REDCap log viewer. Please note that such logs are never removed from the system. Manual steps for log extraction and removal are described under yearly maintenance.
