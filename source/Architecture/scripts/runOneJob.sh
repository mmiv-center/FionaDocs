...
if [[ "${me}" != "processing" ]]; then
    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] Error: This script needs to be run by the processing user."
    exit
fi

input="/var/www/html/applications/Workflows/php/workflow_joblist.jobs"
echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] INFO `cat ${input} | wc -l` job(s) in workflow_joblist.jobs"

storage="/export/Workflows"
ror="/var/www/html/applications/Workflows/php/ror"
...
#        record_id=`jq '.record_id' ${output_json}`
#        event_name=`jq '.event_name' ${output_json}`
#        field_name=`jq '.field_name' ${output_json}`
#        field_value=`jq '.value' ${output_json}`
#
#        if [[ -z $record_id || -z $event_name || -z $field_name || -z $field_value ]]; then
#	    echo "Can't find data for sending to RedCAP"
#        fi
#
#        echo "Found data for RedCAP. Sending ..."
#
#        DATA="token=EBB4AA7203BE357B64E739074D526F72&content=record&action=import&format=json&type=flat&overwriteBehavior=normal&forceAutoNumber=false&data=[{\"record_id\":${record_id},\"redcap_event_name\":${event_name},\"redcap_repeat_instrument\":\"\",\"redcap_repeat_instance\":\"\",${field_name}:${field_value}}]&returnContent=count&returnFormat=json"
#
#        CURL=`which curl`
...
#    $ok = "TRUE"
#    IFS=$'\n'
#    for line in ${outputAllStudyInstanceUID}; do
#	if grep "${line}" "$inputAllStudyInstanceUID"; then
#	    $ok = "TRUE"
#	else
#	    $ok = "${line}"	    
#	fi
#   done
#    # only if the above is true - all generated studyinstanceuids exist in the input we continue

#    if "$ok" = "TRUE"; then
#	echo "ok, only valid data in output folder, send to FIONA"
#	/var/www/html/server/utils/s2m.sh "${folder}/ror/${dirnam}"*/output/
#    else
#	echo "Error: at least one of the StudyInstanceUID's in the output does not appear in the input (${line}). Fix your container! Data is ignored..."
#    fi

    if [ -z "${StudyInstanceUIDError}" ]; then
        echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] only valid data in output folder, send to FIONA..."
	/usr/bin/storescu -nh -aec DICOM_STORAGE -aet FIONA +sd +r -d vir-app5274.ihelse.net 7810 "${ror_folder_path}_output"
	...
	if [ -e "${output_json}" ]; then
	    ...
	    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] Sending to REDCap... sendToREDCap.php \"${project}\" \"${realpath_output_json}\""
	    /usr/bin/php /var/www/html/applications/Workflows/php//sendToREDCap.php "${project}" "${realpath_output_json}"
	    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] Send to REDCap done (\"${project}\" \"${realpath_output_json}\")."
	fi
    else
