...
if [ "$(( $(date +"%s") - $(stat -c "%Y" "$storelog") ))" -lt "$testtime" ]; then
   echo "`date +'%Y-%m-%d %H:%M:%S'`: INFO do not check with storescp: ${storelog} is too new, seems to work" >> $log
   exit 0
fi

echo "`date +'%Y-%m-%d %H:%M:%S'`: INFO try now: /usr/bin/echoscu $PARENTIP $PARENTPORT" >> $log
...
if (($? != 0)); then
   ...
   if [ -z "$pid" ]; then
       echo "`date +'%Y-%m-%d %H:%M:%S'`: INFO storescp's pid could not be found, hope is that in 10min storectl.sh starts it again" >> $log
       /var/www/html/server/bin/storectl.sh stop
      exit 0
   fi
   echo "`date +'%Y-%m-%d %H:%M:%S'`: INFO detected unresponsive storescp, kill \"$pid\" and hope that the system restarts it" >> $log
   ...
   count=20
   while [ ! -z "$portstr" ]; do
      echo "`date +'%Y-%m-%d %H:%M:%S'`: INFO the port is still in use..." >> $log
...
while read -r line; do
	...
	:
        ...
        echo "`date +'%Y-%m-%d %H:%M:%S'`: Error, detectStudyArrival is running for more than 1 hour, stop it now and have it restart" >> $log
