...
Send to me (s2m). Sends a DICOM directory using the dcmtk docker container from the local machine to the local DICOM node. This script can be used to re-classify DICOM files (creates /data/site/raw and /data/site/participant information). In rare cases the docker container might not work. In such cases as a fall-back strategy the systems storescu (DCMTK) executable is used instead.
...
- user: processing
- depends-on:
  - storectrl receiver on FIONA (port 11112)
...
if [[ "$1" == "last" ]]; then
	...
	# Here we use a special storescu.cfg with the Siemens private SOPClass and dcmtk 3.6.2
	/bin/bash -c "docker run --rm -i -v ${dir}:/input dcmtk /bin/bash -c \"/usr/bin/storescu -xf /etc/dcmtk/storescu.cfg Default -v +sd +r -nh $myip $myport /input; exit\" </dev/null"
        if [[ $? -ne "0" ]]; then
